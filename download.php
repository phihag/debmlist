...
function _parse_players($players_html) {
	if (\preg_match_all('/
		<tr>\s*
...
function download_team($tournament_id, $team_id, $team_name) {
	$players_url = _make_url('teamrankingplayers', $tournament_id, '&tid=' . $team_id);
	$players_html = _download_html($players_url);
	...
	$male_players = _parse_players($players_m_m['tbody']);
	...
	$female_players = _parse_players($players_f_m['tbody']);
...
function download_league($url, $league_key) {
	...
	$teams_url = _make_url('draw', $tournament_id, '&draw=' . $groups['draw']);
	$teams_html = _download_html($teams_url);
	...
	$matches_url = _make_url('drawmatches', $tournament_id, '&draw=' . $groups['draw']);
	$matches_html = _download_html($matches_url);
	...
	$teams_info = \array_map(function($t) use ($tournament_id) {
		return download_team($tournament_id, $t['team_id'], $t['name']);
	}, $teams);
...
function main() {
	...
	foreach ($config['leagues'] as $l) {
		$leagues[] = download_league($l['url'], $l['league_key']);
	}

	// TODO write leagues to output file
}
